...
def matrix_cofactor(array):
	...
	mat=array
	array=None
	array = np.linalg.inv(mat).T * np.linalg.det(mat)
	return array
...
def f1(X1,coef1,X2,coef2,seed1,seed2,seed3,shape1,shape2):
    ...
    if( (shape1[0],X1.shape[1]) != (shape2[0], X2.shape[1]) ):
        return -1
    if(X1.shape[0]!=shape1[1]):
        return -1
    if(X2.shape[0]!=shape2[1]):
        return -1
    ...
    result=np.matmul(W1,X1**coef1)+np.matmul(W2,X2**coef2)+B
