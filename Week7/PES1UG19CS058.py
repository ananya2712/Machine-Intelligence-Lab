...
class AdaBoost:
    ...
    def stump_error(self, y, y_pred, sample_weights):
        ...
        miss_Classified = np.where(y_pred!=y,1,0)
        r = np.sum(sample_weights*miss_Classified)/np.sum(sample_weights)
        return r
    ...
    def compute_alpha(self, error):
        ...
        eps = 1e-9
        # TODO
        alph = (0.5)*np.log((1-error)/(error+eps))
        return alph
    ...
    def update_weights(self, y, y_pred, sample_weights, alpha):
        ...
        """
        # TODO
        e = self.stump_error(y, y_pred, sample_weights)
        if(e==0):
        ...
        for i in range(len(sample_weights)):
            if y[i] == y_pred[i]:
                ...
                sample_weights[i] = sample_weights[i]/(2*e)
    ...
    def predict(self, X):
        ...
        """
        # TODO
        s = np.array([self.stumps[stump].predict(X) for stump in range(self.n_stumps)])
        return np.sign(s[0])
    ...
    def evaluate(self, X, y):
        ...
        c = (pred == y)
        acc = np.mean(c) * 100  # accuracy calculation
        print(acc)
        return acc
